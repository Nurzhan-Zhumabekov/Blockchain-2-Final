...
contract PoolHandler is Test {
    ...
    function swapGovForRwa(uint256 amountIn) external {
        ...
        if (gov.balanceOf(msg.sender) < amountIn) { vm.stopPrank(); return; }
    ...
    function swapRwaForGov(uint256 amountIn) external {
        ...
        vm.startPrank(msg.sender);
        if (rwa.balanceOf(msg.sender) < amountIn) { vm.stopPrank(); return; }
    ...
    function addLiquidity(uint256 amount0, uint256 amount1) external {
        ...
        vm.startPrank(msg.sender);
        if (gov.balanceOf(msg.sender) < amount0) { vm.stopPrank(); return; }
        if (rwa.balanceOf(msg.sender) < amount1) { vm.stopPrank(); return; }
...
contract RWAPoolInvariantTest is StdInvariant, Test {
    ...
    function setUp() public {
        ...
        // Explicitly set senders to avoid the pool itself being picked as a caller
        // (pool self-swapping would corrupt reserves via no-op self-transfers).
